...
const weatherIcon = (weather) => {
  if (weather == "Rain") {
    ...
    return <i className="fas fa-sun text-warning mr-1"></i>;
  }
};
const WeatherPresent = ({ weatherData, dailyData }) => {
  return (
    <div className="mt-1 mb-5 pt-1 pb-5 pl-4 text-dark">
      <div className="text-secondary mb-3">
        <h2>
          <i className="fas fa-cloud-sun text-secondary mr-1"></i>
          Hamilton Weather
        </h2>
      </div>
          ...
          <>
            <div className="col-md text-left border-bottom p-3" key={index}>
              <h6 className="text-success">
                {moment(item.dt, "X").format("MMMM Do dddd")}
              </h6>{" "}
              ...
              <h6 className="text-secondary">
                {"Highest: " + Math.round(item.temp.max) + "°C"}
              </h6>
              <h6 className="text-secondary">
                {"Lowest: " + Math.round(item.temp.min) + "°C"}
              </h6>
      ...
      <div className="row">
        {weatherData.map((item, index) => {
          const isValidTime =
            moment(item.dt, "X").format("HH") < 17.1 &&
            moment(item.dt, "X").format("HH") > 6.9;
          const badgeClass =
            index == 0
              ? "badge badge-danger"
              : moment(item.dt, "X").format("HH") > 12 &&
                moment(item.dt, "X").format("HH") < 2
              ? "badge badge-warning"
              : isValidTime
              ? "badge badge-success"
            ...
            <>
              <div className="col-md text-left border-bottom p-3" key={index}>
                <span className={badgeClass}>
                  {moment(item.dt, "X").format("MMMM Do dddd LT")}
                </span>{" "}
                <h6>
                  {moment(item.dt, "X").format("HH") > 11.9 &&
                    moment(item.dt, "X").format("HH") < 14 && (
                      <i className="fas text-warning fa-utensils mr-1"></i>
                    )}
                  {weatherIcon(item.weather[0].main)}
                  {capitalize(item.weather[0].description)}
                </h6>
